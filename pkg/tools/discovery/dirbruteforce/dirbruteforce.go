...
func (d *DirScanner) saveResults() error {
	...
	if _, err := file.WriteString("# Generated by GopherStrike DirBruteForce\n"); err != nil {
		return fmt.Errorf("failed to write to output file: %w", err)
	}
	...
	// Write results
	if _, err := file.WriteString("STATUS  PATH                                               SIZE           TIME\n"); err != nil {
		return fmt.Errorf("failed to write to output file: %w", err)
	}
	...
	for _, result := range d.results {
		if _, err := file.WriteString(fmt.Sprintf("%-6d  %-48s  %-13d  %dms\n",
			result.StatusCode,
			result.Path,
			result.ContentLength,
			result.ResponseTime.Milliseconds())); err != nil {
			return fmt.Errorf("failed to write to output file: %w", err)
		}
	}
