...
func (s *Scanner) saveResults() error {
	...
	if _, err := file.WriteString("# Generated by GopherStrike S3Scanner\n"); err != nil {
		return err
	}
...
func RunS3BucketScanner() error {
	...
	_, err = fmt.Scanln(&useWordlist)
	if err != nil {
		// If there's an error (like empty input), just continue with default
		useWordlist = "n"
	}

	if strings.ToLower(useWordlist) == "y" {
		fmt.Print("[?] Enter wordlist path: ")
		_, err = fmt.Scanln(&options.WordlistPath)
		if err != nil {
			return fmt.Errorf("error reading wordlist path: %v", err)
		}
	}
