// Package subdomain pkg/tools/subdomain/scanner.go
package subdomain
...
func RunScanner() error {
	...
	if _, err := fmt.Scanln(); err != nil {
		// Log the error and continue since this is just a user prompt
...
func saveTextResults(scanCtx *ScanContext, result tools.ScanResult, filename string) error {
	...
	// Write header
	if _, err := fmt.Fprintf(file, "GopherStrike SubTapper - Subdomain Scan Results\n"); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "=============================================\n\n"); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "Target Domain: %s\n", scanCtx.Domain); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "Scan Time: %s\n", time.Now().Format(time.RFC1123)); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "Duration: %.2f seconds\n", result.Duration); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "Wordlist: %s\n", scanCtx.WordlistPath); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "Total Subdomains Found: %d\n", result.TotalFound); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "Active Subdomains: %d\n\n", result.Active); err != nil {
		return err
	}

	// Write result table
	if _, err := fmt.Fprintf(file, "%-40s %-15s %-7s %-20s\n", "SUBDOMAIN", "STATUS", "HTTP", "IP ADDRESSES"); err != nil {
		return err
	}
	if _, err := fmt.Fprintf(file, "%s\n", strings.Repeat("-", 90)); err != nil {
		return err
	}

	// Write each subdomain
	for _, subdomain := range result.Results {
		if err := writeSubdomainToFile(file, subdomain); err != nil {
			return err
		}
	}
	...
	if _, err := fmt.Fprintf(file, "Generated by GopherStrike SubTapper on %s\n", time.Now().Format(time.RFC1123)); err != nil {
		return err
	}
...
func saveCSVResults(_ *ScanContext, result tools.ScanResult, filename string) error {
	...
	// Write CSV header
	if _, err := fmt.Fprintln(file, "Subdomain,Status,HTTP Status,IP Addresses,Error"); err != nil {
		return err
	}
	...
	for _, subdomain := range result.Results {
		...
		if _, err := fmt.Fprintf(file, "%s,%s,%s,%s,%s\n",
			subdomainName, status, httpStatus, ips, errorField); err != nil {
			return err
		}
	}
...
// writeSubdomainToFile writes a single subdomain entry to the result file
func writeSubdomainToFile(file *os.File, subdomain tools.SubdomainResult) error {
	status := "Inactive"
	if subdomain.Active {
		status = "Active"
	}
	...
	_, err := fmt.Fprintf(file, "%-40s %-15s %-7s %-20s\n",
